...
actor_template = ChatPromptTemplate.from_messages(
    [
        ...
        (
            "system", "Answer the user's question above using the required format and make sure to suggest search queries in the required format (if asked for)."
        )
    ]
...
chain_summary = (
    summarize_instruction | llm
)

if __name__ == "__main__":
    response = first_responder.invoke(
        {"messages": [HumanMessage(content="Write about AI-powered SOC / autonomous problem domain, List startups which does this and raises captial.")]}
    )
    print(response)
